...
if __name__ == "__main__":
    ...
    if level == 1:
        ...
        state_df['平仓有无背离'] = '无背离'
        state_df['模型'] = mode
        state_df.to_excel(fold_ini_path + 'state_blue_line//state_singal_symbol_' + str(int(np.around(100000 * fee, 0))) + '.xlsx',
                          encoding='gbk')
    porfolio_state = pd.DataFrame(porfolio_lst, columns=['品种数', 'period', 'fee', 'sharpe_ratio', 'ann_return',
                                                         'max_drawdown', 's_date', 'e_date'])
    porfolio_state.to_excel(fold_ini_path + 'state_blue_line//state_porfolio_' + str(int(np.around(100000 * fee, 0))) + '.xlsx',
                      encoding='gbk')
