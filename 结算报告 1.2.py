...
driver.find_element_by_xpath("/html/body/div[8]/div[2]/div[2]/div[2]/div/div/div/div/div[2]/div[2]/ul/li[2]/a").click()
time.sleep(2)
...
df = pd.read_excel('g://trading//浙商.xlsx')
df2 = pd.read_excel('g://trading//海通.xlsx', encoding='gbk')\
.rename(columns={'最新价': '市价', '浮动盈亏': '盈亏', '证券余额': '股票余额'}).iloc[:, :]
df2 = df2[(df2['证券名称'] != '科创额度')]
# print(df2)
...
for stock_name in df2['证券名称']:
    ...
    if stock_name in df['证券名称'].to_list():
        ...
        stock_price = ((stock_price1 * stock_num1) + (stock_price2 * stock_num2)) / stock_num   # 平均成本
...
for stock_name in df['证券名称']:
    ...
    if stock_name not in df2['证券名称'].to_list():
        ...
        market_cap = df[df['证券名称'] == stock_name]['市值'].iloc[0]
        if 'nan' in str(market):
            stock_cost = stock_cost * exchange_rate
            stock_price = stock_price * exchange_rate
            profit = profit * exchange_rate
...
# 输出
temp[['证券名称', '股票余额', '成本价', '市价', '市值', '盈亏', '盈亏比例', '仓位占比', '行业（申万二级）']].to_excel(export_file_path)
